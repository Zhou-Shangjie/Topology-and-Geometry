...
\end{equation}
\begin{property}
    The transition functions satisfy the following relation 
    \begin{subequations}
        \begin{align}
...
\begin{definition}[Principal Bundle]
    In \cref{def:fiberbundle}, if the fiber $F$ is identical to the structure group $G$, then the fiber bundle is called a \textbf{principal bundle}.
...
The action of $G$ on $U\pi^{-1}(U_i)$ is defined by 
...
\begin{proof}
    ...
    \end{equation}
    %where we can verify that $t_{ji}(p)\equiv\phi_{i,p}^{-1}\circ\phi_{j,p}=g_j g_i^{-1}$
\end{proof}
