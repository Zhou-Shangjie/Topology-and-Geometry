...
\begin{definition}[Homotopy\label{def:higherhomotopy}]
...
\begin{definition}[$n$th Homotopy Group\label{def:higherhomotopygp}]
    Let $X$ be a topological space.
...
There is a vector space isomorphism $\#:\mathbb{g}\to V_u P$ given by $A\mapsto A^\#$.
...
\begin{definition}[Connection\label{def:principal_connection}]
...
\begin{definition}[Connection One-Form]
    A connection one-form $\omega$ (or \textbf{Ehresmann connection}) is a Lie-algebra-valued one-form $\omega\in\mathfrak{g}\otimes T_u^\ast P$ that satisfies
    \begin{subequations}
        \begin{align}
    ...
    \end{subequations}
\end{definition}
After choosing a connection one-form $\omega_u$ for any $u\in P$, we have induced horizontal subspaces which are defined to be the kernel of $\omega_u$
...
There is a Lie-algebra-valued one-form $\mathcal{A}_i$ on $U_i$ naturally induced by $\sigma_i$:
\begin{equation}
    \mathcal{A}_i\equiv\sigma_i^\ast\omega
\end{equation}
